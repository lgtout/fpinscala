...
object TestIsSorted {
  def main(args: Array[String]): Unit = {
    val comparator = { (a: Int, b: Int) => b > a }

    var array = Array(0,1,2,3)
    var sorted = PolymorphicFunctions.isSorted(array, comparator)
    println("array: %s".format(array.toList))
    println("expected: true")
    println("actual: %s".format(sorted))

    println()

    array = Array(0,2,1,3)
    sorted = PolymorphicFunctions.isSorted(array, comparator)
    println("array: %s".format(array.toList))
